...
pragma(ctfe) public char[][] xposeAttribParser_overload(char[] foo) {
	return [
		"!type", foo,
		"typeName", '"' ~ foo ~ '"'
	];
...
pragma(ctfe) private char[] escapeDoubleQuotes(char[] str) {
	char[] res;
	foreach (c; str) {
		switch (c) {
			case '\\': {
...
pragma(ctfe) char[] attribCodegen(char[] name, char[][] parsed) {
	name = escapeDoubleQuotes(name);
	
	char[] res = 
		`pragma(ctfe) interface `~name~` {
...
pragma(ctfe) private char[] unescape(char[] str) {
	bool any = false;
	foreach (c; str) {
		if ('\\' == c) {
			any = true;
...
pragma(ctfe) private char[][] splitStrByChar(char[] str, char d) {
	char[][] res;
	int i = 0;
	int prev = 0;
	bool escape = false;
...
pragma(ctfe) private bool isSpaceCT(char c) {
	return ' ' == c || '\t' == c;
}

pragma(ctfe) private char[] striplCT(char[] s) {
	uint i;
...
pragma(ctfe) private char[] striprCT(char[] s) {
	uint i;
...
pragma(ctfe) private char[] stripCT(char[] s) {
    return striprCT(striplCT(s));
}


pragma(ctfe) private bool isLetterCT(char c) {
	return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z');
}


pragma(ctfe) private bool isDigitCT(char c) {
	return c >= '0' && c <= '9';
}


pragma(ctfe) private int locateCT(char[] str, char c) {
	foreach (i, c2; str) {
		if (c == c2) return i;
	}
	return str.length;
...
pragma(ctfe) private char[] parseAttribsWorker(ref char[] str, int depth = 0, char[] prefix = "") {
	char[] code;
	...
	while (str.length > 0) {
		...
		if (singleField) {
			...
			code ~= `~"pragma(ctfe) interface _`~scopeName~`{ enum : bool { isSimple = false } const name = \"`~scopeName~`\"; "`;
...
pragma(ctfe) private char[] parseAttribs(char[] str) {
	char[] code = `"pragma(ctfe) interface _attribs {"`;
...
pragma(ctfe) private void splitNameAttribs(char[] line, out char[] name, out char[] attribs) {
	int i = 0;
	for (; i < line.length; ++i) {
...
pragma(ctfe) private char[][] splitLines(char[] str) {
	char[][] res;
	while (str.length) {
		int from = -1, to = -1;
...
pragma(ctfe) private void findFieldXposeRename(char[] str, out char[] dname, out char[] xname) {
	foreach (i, c; str) {
		if (':' == c) {
			dname = str[0..i];
			xname = str[i+1..$];
...
pragma(ctfe) char[] xpose2MainCodegenWorker(char[] targetName, char[] dname, char[] dataPrefix, char[] xname, char[] reflName, char[] attribsCode) {
	if ("_ctor" == reflName) {
			return "
				res ~=
				`pragma(ctfe) interface _Field_"~xname~" {
...
pragma(ctfe) private bool isPatternCT(char[] str) {
	foreach (c; str) {
		switch (c) {
			case '.':
			case '?':
...
pragma(ctfe) char[] xpose2MainCodegen(alias targetAlias)(char[] funcSuffix, char[] dataPrefix, char[] metaName, char[] expStr) {
	char[] res =
	"pragma(ctfe) static char[] _xposeDataCodegenFunc"~funcSuffix~"() {
		char[] res;";
...
pragma(ctfe) char[] xpose2(char[] expStr) {
	return ctReplace(
...
pragma(ctfe) char[] xpose2(char[] target, char[] expStr) {
	return ctReplace(
