...
version (linux) {
	char[] configFilePath = `/etc/decant.cfg`;
}
else {
	char[] configFilePath = `decant.cfg`;
}


char[][] getAllArgs(char[][] cmdargs) {
	char[] configData;
	try {
		configData = cast(char[])std.file.read(configFilePath);
	} catch (Exception e) {
		writefln("Warning: Could not open config file: '%s'", configFilePath);
	}

	char[][] lines = std.string.splitlines(configData);

	foreach (inout l; lines) {
		l = std.string.strip(l);
	}
...
-stuff-dir=dir        // directory with templates and stuff
...
Segmentation fault`;
	...
	foreach (arg; args) {
		...
		else if (getArg("-stuff-dir=", arg, stuffDir)) {
			// nothing
		}
		else {
			writefln("Warning: Unrecognized arg: ", arg);
		}
	}


	try {
		dKeywords1 = std.string.splitlines(cast(char[])std.file.read(std.path.join(stuffDir, "decantKeywords1.txt")));
		dKeywords2 = std.string.splitlines(cast(char[])std.file.read(std.path.join(stuffDir, "decantKeywords2.txt")));
		dKeywords3 = std.string.splitlines(cast(char[])std.file.read(std.path.join(stuffDir, "decantKeywords3.txt")));
		dKeywords4 = std.string.splitlines(cast(char[])std.file.read(std.path.join(stuffDir, "decantKeywords4.txt")));
	} catch (Exception e) {
		throw new Exception("could not read one of decantKeywords[1-4].txt files");
	}


	char[] preTempl, postTempl;
	if (addTemplate) {
		preTempl	= cast(char[])std.file.read(std.path.join(stuffDir, "decantTemplate.pre")).dup;
		postTempl	= cast(char[])std.file.read(std.path.join(stuffDir, "decantTemplate.post")).dup;
	}
...
private TokenizerRule[] readRules(char[] rulesFile=null)
{
    if (rulesFile is null) {
        rulesFile = std.path.join(stuffDir, `decant.rules`);
    }
