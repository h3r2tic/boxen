...
///////////////////////////////////////////////////////////////////////////////
function Outline(candyPath)
{
	this.candyPath		= candyPath;
    this.tree           = new TreeView(this.candyPath, false);
        ...
        for (var i = 0; i < this.mountPoint.children.length; ++i)
        {
            ...
            if (n.firstChild.nodeName == "#text")
            {
                ...
                if ( this.classRegExp.test(text) )
                    iconSrc = candyPath + "img/outline/class.gif";
                else if ( this.structRegExp.test(text) )
                    iconSrc = candyPath + "img/outline/struct.gif";
                else if ( this.enumRegExp.test(text) )
                    iconSrc = candyPath + "img/outline/enum.gif";
                else if ( this.templateRegExp.test(text) )
                    iconSrc = candyPath + "img/outline/template.gif";
                else if ( this.aliasRegExp.test(text) )
                    iconSrc = candyPath + "img/outline/alias.gif";
                else // function or variable? check whether '(' ')' exists on the right
                {
                    ...
                    {
                        iconSrc = candyPath + "img/outline/func.gif";
                    }
                    else
                        iconSrc = candyPath + "img/outline/var.gif";
                }
            }
            else // enum member ?
                iconSrc = candyPath + "img/outline/var.gif";
...
///////////////////////////////////////////////////////////////////////////////
function PackageExplorer(candyPath)
{
	this.candyPath = candyPath;
    this.tree = new TreeView(this.candyPath, true);
 
    this.addModule = function(mod)
    {
        var moduleIco = candyPath + "img/outline/module.gif";
        var packageIco = candyPath + "img/outline/package.gif";
        ...
        for (var i = 1; i < path.length; ++i)
        {
            ...
            if (path.length == i + 1)
                node.setRef(candyPath + "../" + mod.replace(/\./g, "/") + ".html");
        }
    }
}
...
function Explorer()
{
    ...
    this.initialize = function(moduleName, candyPath)
    {
		this.candyPath = candyPath;

	    this.packageExplorer = new PackageExplorer(this.candyPath);
	    this.outline = new Outline(this.candyPath);
        ...
        this.outline.tree.createBranch( moduleName, this.candyPath + "img/outline/module.gif" );
