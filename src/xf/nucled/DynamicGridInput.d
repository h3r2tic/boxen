module xf.nucled.DynamicGridInput;
		...
		gui.openOverlay();
			with (VBox().cfg(`layout = { padding = 8 8; }`) [{
				with (Label()) {
					text = popupMsg;
					fontSize = 11;
					style.color.value = vec4(1, 1, 1, 0.8);
				}
			}]) {
				parentOffset = pos;// - grp.globalOffset;
				style.background.value = popupBack;
				style.border.value = popupBorder;
			}
		//}];
		gui.close;		// overlay
	}
			...
			for (int y = 0; y < rows+1; ++y) {
				...
				HBox(y).spacing(cellSpacing.x) [{
					for (int x = 0; x < cols; ++x) {
						...
						if (popupCol == x && popupRow == y) {
							popupCoords = input.globalOffset + vec2(1, 1 + input.size.y);
						}
					}
				}];
