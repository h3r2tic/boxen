...
def parseEnum(enumMatch, lineIter):
    ...
    for enumDef in lineIter.iterAllLines():
        for m in rematch(r"(\w+(?:, \w+)*) enum:(.*)$", enumDef):
...
def parseEnumEntity(enumDef, addItem):
    ...
    for m in rematch(r"use (\w+) (\w+)$", enumDef):
        ...
        addItem(NameAlias(m.group(1), m.group(2)))
        return

    for m in rematch(r"(\w+) = ((?:0x[0-9A-F]+|[0-9]+)[ul]*)( #.*)?$", enumDef):
        ...
        for alias in aliases:
            addItem(DirectName(alias, m.group(2), annot))
        
        return
        
    for m in rematch(r"(\w+) = (\w+)$", enumDef):
        assert 2 == len(m.groups())
        addItem(SpecialNameAlias(m.group(1), m.group(2)))
        return
