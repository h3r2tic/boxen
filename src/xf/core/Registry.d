...
*/
pragma(ctfe) char[] Implements(char[] T) {
...
*/
pragma(ctfe) char[] CtorParams(char[] params) {
...
struct create(T) {
	private struct Named {
		...
		T opCall(classCtorParams!(T) args) {
			...
			if (size_t.max == idx) {
				throw new ClassRegistryException("No implementations registered for '" ~ T.stringof ~ "' (looking for '"~name~"')");
			}
